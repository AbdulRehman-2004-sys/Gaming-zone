...
import { 
    ...
    Loader2, Package, Cpu, ArrowLeft, ChevronDown, Check
} from "lucide-react";
...
export default function GamingGearPage() {
    const mainCategory = "Gaming Gear";
    ...
    async function loadProducts() {
        ...
        try {
            ...
            // Filter products for this specific main category
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
                        <Cpu className="text-yellow-400" /> {mainCategory}
                    </h1>
                    <p className="text-zinc-500 mt-1">Manage and organize {mainCategory.toLowerCase()} inventory.</p>
                </div>
                <button 
                    onClick={() => {
                        setEditingProduct(null);
            ...
            {loading ? (
                ...
                <div className="bg-zinc-900/50 border-2 border-dashed border-zinc-800 rounded-3xl p-20 text-center">
                    ...
                    <p className="text-zinc-500 mt-2">Start by adding your first product to this category.</p>
                </div>
            ) : (
                        ...
                        <Card key={product._id} className="bg-zinc-900 border-zinc-800 overflow-hidden group hover:border-yellow-400/30 transition-all duration-300 shadow-xl">
                            <div className="aspect-square relative overflow-hidden bg-zinc-950">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                    <p className="text-zinc-400 text-[10px] line-clamp-2 italic">{product.description}</p>
                                </div>
                            </div>
