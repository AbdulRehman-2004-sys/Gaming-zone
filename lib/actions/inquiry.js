...
export async function getInquiriesByEmail(email) {
  try {
    await connectDB();
    const inquiries = await Inquiry.find({ email: email.toLowerCase() }).sort({ createdAt: -1 });
    return JSON.parse(JSON.stringify(inquiries));
  } catch (error) {
...
export async function markAsRead(id) {
  try {
    await connectDB();
    const inquiry = await Inquiry.findByIdAndUpdate(id, { status: "read" }, { new: true });
    ...
    if (inquiry) {
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 25px; border-radius: 12px; background-color: #fcfcfc;">
            <div style="text-align: center; margin-bottom: 25px;">
              <h2 style="color: #facc15; font-size: 22px; text-transform: uppercase; margin: 0;">Status Update</h2>
              <p style="color: #666; font-size: 14px;">Your ticket is being reviewed</p>
            </div>
            <p>Hi ${inquiry.name},</p>
            <p>Good news! Our support team has just reviewed your message regarding: <br /><strong>"${inquiry.subject}"</strong></p>
            <div style="background: #ffffff; border-left: 4px solid #facc15; padding: 15px; margin: 20px 0; font-style: italic; color: #555;">
              "We've received your inquiry and are currently working on a solution. You can expect a detailed response shortly."
            </div>
            <p>You can check the latest status of your ticket anytime on our support page using your email.</p>
            <br />
            <p style="margin: 0;">Best Regards,</p>
            <p style="margin: 0;"><strong>The DROP ZONE Team</strong></p>
          </div>
        `,
